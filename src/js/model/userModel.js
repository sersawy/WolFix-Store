...
const validation = function (user, type = 0) {
  console.log(type);
...
export function checkPassword(password) {
  ...
  if (currentUser.password !== password)
    throw new AppError('setting', 'currentPassword', 'Incorrect current password.');
}
...
export function updateUserPassword(oldPassword, newPassword, confirmPassword) {
  ...
  const currentUser = getCurrentUser();
  validation({ password: newPassword, confirmPassword }, 3);
  const users = getAllUsers();
  const user = users.find((u) => u.email === currentUser.email);
  currentUser.password = user.password = newPassword;
  setCurrentUser(currentUser);
  localStorage.setItem('users', JSON.stringify(users));
}
